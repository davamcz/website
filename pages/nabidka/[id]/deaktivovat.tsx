...
export default withApollo(() => {
  ...
  const { data: offerData, loading: offerLoading, error: offerError } = useOfferQuery({
    variables: { id: offerId },
  })
  const offer = offerData?.offer
  const isActive = offer?.active
  const [changeActiveStateOfferMutation, { loading, error }] = useChangeActiveStateOfferMutation()
  const values = {
    offerId,
    confirmationHash,
    active: !isActive
      ...
      {offer && 
        <Container style={{ maxWidth: '420px' }}>
          ...
          {error && 
            <>
              <Message>
                Něco se nepovedlo. Zkuste to, prosím, znovu a nebo nám dejte vědět na info@davam.cz
                a my to zařídíme postaru ručně.
              </Message>
              <Spacer />
            </>
          }
